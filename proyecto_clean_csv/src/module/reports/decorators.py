...
def track_changes(_func: Callable | None=None,*,action: str|None=None):
    ...
    def decorator(func: Callable):
        def wrapper(*args, **kwargs):
            ...
            if action:
                logger.info(
                    "Se inicia la ejecución de la función '%s'. Acción: %s.",
                    func.__name__,
                    action
                )
            else:
                logger.info(
                    "Se inicia la ejecución de la función '%s'.",
                    func.__name__
                )
            ...
            logger.info(
                 "Tiempo de ejecución de '%s': %.4f segundos.",
                func.__name__,
                duration
                )
...
def track_dtype_changes(func: Callable)-> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        tipos_despues=result.dtypes.to.dict()

        logger.info(
            "Resumen de cambios de tipos en la función '%s':",
            func.__name__
        )
        for col in tipos_despues:
            ...
            logger.info(
                "Columna '%s' → Tipo antes: %s → Tipo después: %s",
                col,
                tipo_antes,
                tipo_despues
            )
